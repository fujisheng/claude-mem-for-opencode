...
function getBaseDir() {
  // Try to find the claude-mem plugin directory
  const possiblePaths = [
    // When running from project root
    path.join(process.cwd(), '.opencode', 'plugins', 'claude-mem', 'vendor', 'claude-mem', 'plugin', 'scripts'),
    // When running from .opencode/skills/mem-search
    path.join(process.cwd(), '..', '..', 'plugins', 'claude-mem', 'vendor', 'claude-mem', 'plugin', 'scripts'),
    // When running from .opencode/plugins/claude-mem/.opencode
  ...
  return path.join(process.cwd(), '.opencode', 'plugins', 'claude-mem', 'vendor', 'claude-mem', 'plugin', 'scripts');
}

const BASE_DIR = getBaseDir();
const WORKER_SCRIPT = path.join(BASE_DIR, 'worker-wrapper.cjs');
...
async function main() {
  ...
  log(`Spawning worker: ${WORKER_SCRIPT}`);
  const worker = spawn('bun', [WORKER_SCRIPT], {
    cwd: BASE_DIR,
